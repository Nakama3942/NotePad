...
void SheetOfNotes::createFile(QString pathtofile, QString namefile)
{
    ...
    *filename = namefile;
    ...
    //Если открывать большое HTML файл, то показывается только его первая
    //половина, однако при переключении на вид Документа (изменение
    //ширины) - документ отображается весь. Следующие два метода автоматически
    //уменьшают, а потом возвращают размер поля ввода, чтобы прогрузить весь файл
    ui->winNote->setMinimumSize(ui->winNote->minimumWidth() - 1,
                                ui->winNote->minimumHeight());
}
